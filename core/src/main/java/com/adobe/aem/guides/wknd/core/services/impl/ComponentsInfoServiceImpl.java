...
@Component(immediate = true, service = ComponentsInfoService.class, name = "com.adobe.aem.guides.wknd.core.services.ComponentsInfo", configurationPid = "com.adobe.aem.guides.wknd.core.services.impl.ComponentsInfoImpl", property = {
        "service.vendor=GAO WKND",
        "service.description=GAO WKND Training" }, configurationPolicy = ConfigurationPolicy.OPTIONAL)
public class ComponentsInfoServiceImpl implements ComponentsInfoService {
    ...
    public List<Map<String, String>> getComponents(final String path) {
        LOG.info("\n -------- getComponents GET COMPONENT METHOD----------- ");
        // local variables
        ...
        try {
            ...
            long hitNum = 0;
            LOG.info("\n ---getComponents TOTOAL HIT ---> " + result.getTotalMatches());
            ...
            while (componentResource.hasNext()) {
                ...
                if (component.getValueMap().get("componentGroup") != null) {
                    componentGroup = component.getValueMap().get("componentGroup").toString();
                }
            ...
            LOG.error("111 LoginException occurred exception :{}", e);
        } catch (Exception e) {
            LOG.error("112 LoginException occurred exception :{}", e);
        } finally {
    ...
    public String getComponentReferenceCount(final String component, Session session) {
        LOG.info("\n -------- getComponentReferenceCount GET COMPONENT METHOD----------- ");
        // local variables
        final List<Map<String, String>> componentList = new ArrayList<Map<String, String>>();
        long references = 0;
        ResourceResolver resolver = null;
        try {
            ...
            LOG.info("\n ---getComponentReferenceCount TOTOAL HIT ---> " + result.getTotalMatches());
            // Iterator<Resource> componentResource = result.getResources();

        } catch (LoginException e) {
            LOG.error("221 LoginException occurred , exception :{}", e);
        } catch (Exception e) {
            LOG.error("222 LoginException occurred , exception :{}", e);
        } finally {
            if (resolver != null && resolver.isLive()) {
                resolver.close();
            } // if{}
    ...
    public List<Map<String, String>> getComponentReferences(final String component) {
        LOG.info("\n -------- getComponentReferences GET COMPONENT METHOD----------- ");
        // local variables
